...
sub vcl_recv {
    ...
    if (req.http.cookie) {
        ...
        set req.http.cookie = regsuball(req.http.cookie, ";(MOCKSESSID[^=]*)=", "; \1=");
        set req.http.cookie = regsuball(req.http.cookie, ";[^ ][^;]*", "");
        set req.http.cookie = regsuball(req.http.cookie, "^[; ]+|[; ]+$", "");
    ...
    if (req.http.host ~ "intranet.toutsurmoneau.test" || req.http.host ~ "riuc-admin.toutsurmoneau.fr"){
        return (pass);
    }
